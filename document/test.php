...
<form  method="POST">
    ...
    <div id='rechercher'>
       <br> <br> <h3>Rechercher un etudiant</h3>
      <p> 
        <input type="text" name="nom"  placeholder="Rechercher Nom Etudiant" size="20" />
      </p>
      <p> 
        <input type="text" name="prenom"  placeholder="Rechercher Prenom Etudiant" size="20" />
      </p>
...
if(!empty($_POST['nom']) && !empty($_POST['prenom']) && isset($_POST['rechercher'])){
    
    //$r=new controlEtudiant();
    //$r->findEtudiant();
    $con=new testconnexion;
    $bdd=$con->connect();
$rec=$bdd->prepare("SELECT `id_etudiant`, `matricule`, `nom`, `prenom`, `email`, `telephone`, `date_de_naissance` FROM `ETUDIANT` WHERE nom=? AND prenom=?");

$rec->execute(array( $_POST['nom'],$_POST['prenom'] ));
$array= $rec->fetch();
...
<tr>';
  ...
  echo '<td>'. $array['matricule'].'</td> <td>'. $array['prenom'] .'</td> <td>'. $array['nom'] .'</td> <td>'. $array['email'] .'</td> <td>'.  $array['telephone'] .'</td> <td>'.  $array['date_de_naissance'] .'</td>
